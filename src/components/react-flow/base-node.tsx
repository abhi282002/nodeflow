...
export const BaseNode = forwardRef<HTMLDivElement, BaseNodeProps>(
  ({ className, status, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'relative rounded-md border bg-card border-muted-foreground text-card-foreground',
        'hover:bg-accent',
    ...
    >
      ...
      {status === 'loading' && (
        <Loader2Icon className="absolute -right-0.5 -bottom-0.5  size-2 text-green-700 stroke-3 animate-spin" />
      )}
    </div>
  ),
);
